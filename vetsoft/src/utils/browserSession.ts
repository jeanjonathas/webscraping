...
export async function getBrowser(showBrowser: boolean = false): Promise<Browser> {
  // Verificar se estamos em ambiente Docker (sem servidor X)
  const isDocker = process.env.DOCKER_ENV === 'true' || process.env.CONTAINER_ENV === 'true';
  
  // Se showBrowser for true e não estamos em Docker, forçamos o modo não-headless
  // Em Docker, sempre usamos headless para evitar erros de servidor X
  // Caso contrário, usamos a configuração do ambiente
  const headless = isDocker ? true : (showBrowser ? false : config.browser.headless);
  ...
  if (!browser) {
    ...
    browser = await chromium.launch({
      ...
      args: [
        '--disable-dev-shm-usage', // Evita erros de memória compartilhada em Docker
        '--no-sandbox', // Necessário para Docker
        ...(headless ? [] : ['--start-maximized']) // Maximiza a janela quando não for headless
      ]
    });
    isLoggedIn = false;
    browserStartTime = Date.now();
