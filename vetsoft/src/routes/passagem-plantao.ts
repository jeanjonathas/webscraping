  ...
  try {
    ...
    while (currentRetry <= maxRetries) {
      try {
        ...
        // para a operação de inserção de passagem de plantão
        result = await withLockWait(operationId, async () => {
          try {
            // Navegar para a ficha de internação
            console.log(`Navegando para a ficha de internação do animal ${cod_animal}...`);
            await page.goto(`https://dranimal.vetsoft.com.br/m/internacoes/ficha.animal.php?cod_animal=${cod_animal}&cod_internacao=${cod_internacao}#list/page/1/`, {
            ...
            const url = page.url();
            if (!url.includes(`cod_animal=${cod_animal}`) || !url.includes(`cod_internacao=${cod_internacao}`)) {
            ...
            console.error('Erro durante a execução da passagem de plantão:', innerError);
            // Propagar o erro para ser tratado no catch externo
            throw innerError;
          }
